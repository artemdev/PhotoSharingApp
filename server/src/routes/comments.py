...
async def update_comment(comment_id: int, comment: CommentUpdate, db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
    return await CommentRepository.update_comment(db, comment_id, comment, current_user.id)
